...
const MODES: { id: SimulatorMode; label: string; icon: IconType; color: string }[] = [
  {
    id: "design",
    label: "DESIGN",
    icon: RiFlowChart,
            ...
            {MODES.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg font-mono text-xs transition-all ${mode === m.id
                    ? 'border'
          ...
          <div className="flex gap-1.5">
            {(['high-level', 'detailed'] as ViewMode[]).map((v) => (
              <button
                ...
                className={`flex-1 py-1.5 rounded font-mono text-[10px] transition-all ${viewMode === v
